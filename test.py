...
print(device_name)
CVL_root = "path/to/CVL_100/test_WI300/"
IAM_root = "path/to/IAM_100/test_WI300/"
mixed_root = "path/to/Mixed_padded/Mixed/test_WI300/"
WD_root = "path/to/CVL_100/test_WD/"

ckpoint = "path/to/model_checkpoint"
...
class MultipleInputGenerator(Sequence):
	...
	def __init__(self, root):
		...
		:param root: root of the test set.
		"""
		self.genX1 = generator.flow_from_directory(root, target_size=(256, 256), batch_size=1,
...
test_mixed = MultipleInputGenerator(root=mixed_root)
test_CVL = MultipleInputGenerator(root=CVL_root)
test_IAM = MultipleInputGenerator(root=IAM_root)
test_WD = MultipleInputGenerator(root=WD_root)
...
print(model.summary())
print(f"Testing checkpoint {ckpoint}")
...
for i in range(10):
	print("--------------------------------------")
	print("Running test on Mixed...")
	mixed.append(model.evaluate(test_mixed, verbose=2)[1])

	print("Running test on CVL...")
	cvl.append(model.evaluate(test_CVL, verbose=2)[1])

	print("Running test on IAM...")
	iam.append(model.evaluate(test_IAM, verbose=2)[1])

	print("Running test on WD...")
	wd.append(model.evaluate(test_WD, verbose=2)[1])
